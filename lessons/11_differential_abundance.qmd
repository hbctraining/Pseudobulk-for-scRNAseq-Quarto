...
To determine which celltypes are significantly different across our groups, we use the `propeller()` function. After supplying the seurat object as an argument, we then specify the sample IDs, celltypes, and experimental conditions:
...
```{r}
#| label: inspect_propeller_results
#| eval: false
...
```{r}
#| label: table_propeller_results
#| echo: false
library(DT)
datatable(propres)
```

As a final step, we run variance stabilization and fit a linear model to our data to ultimately **create a dataframe of celltype proportions in each sample**. 

```{r}
#| label: transform_propeller_results
...
```{r}
#| label: plot_distributions_celltypes
...
```{r}
#| label: barplot_mean_proportions
...
# [**Exercise 1**](11_differential_abundance_Answer-key.qmd#exercise-1)
...
```{r}
#| label: sccomp_boxplots
# Create sccomp boxplots
sccomp_result %>% 
...
```{r}
#| label: sccomp_propeller_shared_results
# Get significant celltypes from propeller
p_results <- propres %>%
