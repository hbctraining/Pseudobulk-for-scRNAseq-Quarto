...
::: callout-tip
If you haven't done this already, the project can be accessed left-clicking [this link](https://www.dropbox.com/scl/fi/c3ggrdttuk3cqovqocy1a/Pseudobulk_workshop.zip?rlkey=nehku3i8mrtkbibe4wvt04n40&st=t8noj7w8&dl=1).
:::
...
<p align="center">
<img src="../img/R_script_creation_2.gif" width="1000">
</p>
...
We generated this Seurat object using the files that were deposited to GEO. More details on how this Seurat object was generated are detailed in [another lesson](Aside_gen_seurat_object.qmd), where we provide more clarity on what parameters were used for normalization, integration, and clustering. 
...
```{r fig.width=10}
#| label: dimplot_condition_sample
...
Initially, cells were clustered at a range of resolutions. The authors chose a resolution 1.2 as it grouped cells adequately into the celltypes of interest.
...
The mapping of cluster to celltypes is being represented here as a table.

```{r fig.width=10}
#| label: mapping_celltypes_to_clusters
...
cluster_celltype_map <- seurat@meta.data %>%
...
This provides a quick way to see which clusters have been annotated as which celltype.

```{r}
#| label: view_celltype_cluster_map
#| eval: false
View(cluster_celltype_map)
```

```{r}
#| label: DT_celltype_cluster_map
#| echo: false
library(DT)
datatable(cluster_celltype_map)
```

With these updated celltype labels, we can see how they distribute on the UMAP. 

```{r}
#| label: UMPA_with_celltype_labels
# UMAP celltype
...
```{r}
#| label: barplot_proporion_celltypes
