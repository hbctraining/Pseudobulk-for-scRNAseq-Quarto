...
# Extract the genes column
genes <- genes$gene

# Print the top 6 significant genes
genes
```
...
```{r}
#| label: create_expression_scatterplot_function
# Pseudobulk scatterplot of normalized expression
...
```{r}
#| label: create_panelled_plot
# Create empty list to hold plots
plot_list <- list()

# Use pseudobulk scatterplot function to populate list
for (gene in genes) {
  plot_list[[gene]] <- plot_pb_count(dds, gene)
}

# Create multi-panel scatterplot of expression figure
