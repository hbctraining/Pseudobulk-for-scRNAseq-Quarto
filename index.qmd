...
**(2)** Install the 8 packages listed below from **Bioconductor** using the the `BiocManager::install()` function.
...
::: callout-note
There are two ways to install `sccomp`, but both ways will require that you install `cmdstanr`. The instructions for installing `sccomp` using Bioconductor are below:

```{r}
#| label: install_sccomp_bioc
#| eval: false
...
If you run into issues installing `sccomp` via Bioconductor, you can try installing it from GitHub by using `devtools`:

```{r}
#| label: install_sccomp_github
#| eval: false
